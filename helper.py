...
def get_train_test_split_small() -> tuple[npt.NDArray[np.float64], npt.NDArray[np.int64], npt.NDArray[np.float64], npt.NDArray[np.int64], list[str]]:
    ...
    """
    path =''
    df_cor_n = pd.read_csv(path+'data/correlation_n.csv') # Read negative crosstalks
    df_cor_p = pd.read_csv(path+'data/correlation_p.csv') # Read positive crosstalks
    df_cor_n = df_cor_n[['ac_loc','pho_loc', 'cor']] # Select desired columns
    df_cor_p = df_cor_p[['ac_loc','pho_loc', 'cor']] # Select desired columns
    df_cor = pd.concat([df_cor_n, df_cor_p], axis=0)

    crosstalks = {}
    for i in tqdm(range(df_cor.shape[0]), desc='Loading files from disk'):
        crosstalks[i] = df_cor[i:i+1]
    ...
    X, y = df_features.to_numpy(), df_features['cor'].to_numpy()
    y = crosstalk_main.binarize_values(y)
